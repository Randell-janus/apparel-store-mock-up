...
export default function FilterSelect() {
  const { sortFilters, sortFilter, setSortFilter, setSortRadio, getProducts } =
    useProducts();

  const filterOptions = ["Latest Addition"];
  ...
  useEffect(() => {
    if (sortFilter == "Latest Addition") {
      setSortRadio("Latest Addition");
      // getProducts();
    } else {
      setSortRadio(sortFilter);
    }
  }, [sortFilter]);
      ...
      <Listbox value={sortFilter} onChange={setSortFilter}>
        <div className="relative mt-1">
          <Listbox.Button className="relative w-full py-2 pl-3 pr-10 text-left bg-gray-100 rounded-lg cursor-default focus:outline-none focus-visible:ring-2 focus-visible:ring-opacity-75 focus-visible:ring-white focus-visible:ring-offset-orange-300 focus-visible:ring-offset-2 focus-visible:border-indigo-500">
            <span className="block truncate">Sort Filters</span>
